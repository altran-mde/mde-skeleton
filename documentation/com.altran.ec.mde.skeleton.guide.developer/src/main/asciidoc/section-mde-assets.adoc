...
<3> The contents of the file are generated in the {giturl}/plugins//org.eclipse.xtext.example.fowlerdsl/src/org/eclipse/xtext/example/fowlerdsl/generator/StatemachinePolviGenerator.xtend#L30[StatemachinePolviGenerator#toJavaCode(Statemachine)] method. +
+
TIP: For more information on how to implement model-to-text generators with Xtend, please read the https://gitlab.acidspace.nl/ec-mde/xtend-based-generators/-/blob/master/xtend-based-generators/doc/xtend-based-generators.adoc[Xtend-based Model-to-Text Generators Guide].
