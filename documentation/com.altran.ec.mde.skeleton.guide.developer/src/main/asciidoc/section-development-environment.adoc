...
NOTE: TODO explain https://nvie.com/posts/a-successful-git-branching-model/[Gitflow Workflow]
...
The easiest way to create an Oomph setup file is to download the link:{gitlab-url}/{oomph-setup-file-path}[setup file of this {project-name}^] and use it as a base.
...
... Resource URIs: `link:https://gitlab.acidspace.nl/{gitlab-project}/-/raw/{gitlab-branch}/{oomph-setup-file-path}?oomph_form=b%27users/sign_in%27#/[]`
... Click btn:[OK] +
+
...
In this case the setup file can also be link:https://gitlab.acidspace.nl/{gitlab-project}/-/raw/{gitlab-branch}/{oomph-setup-file-path}?inline=false[downloaded] and added by clicking btn:[Browse File System...].
... Enter your GitLab credentials when asked for
.. Select `{project-name}` in the tree
.. Select stream `{gitlab-branch}` in the table
.. Click btn:[Next >]
