...
IMPORTANT:  +
Before creating an initial directory structure, please make sure that these branches are available in the repository and configured correctly, i.e., both protected and `develop` marked as the default branch.
...
The bootstrapped application will only contain a developer documentation plugin that provides the instructions on <<setup-development-environment, setting up a development environment>>.

The listing below shows how to use the EMF Reference Project bootstrap script.
In this example, the https://gitlab.acidspace.nl/ec-mde/larch[LArch] repository is used and the callouts below the listing explain the steps in more detail.

.Bootstrapping an ACIDSpace git repository to build an Eclipse RCP
[source,shell]
----
user@machine MINGW64 ~/git
$ git clone git@gitlab.acidspace.nl:ec-mde/larch.git <1>
Cloning into 'larch'...
...
user@machine MINGW64 ~/git
$ git clone git@gitlab.acidspace.nl:ec-mde/mde-skeleton.git -b BOOTSTRAP-2020-06 <2>
Cloning into 'mde-skeleton'...
remote: Enumerating objects: 1361, done.
remote: Counting objects: 100% (4/4), done.
remote: Compressing objects: 100% (4/4), done.
remote: Total 1361 (delta 0), reused 0 (delta 0), pack-reused 1357
Receiving objects: 100% (1361/1361), 882.60 KiB | 1.19 MiB/s, done.
Resolving deltas: 100% (615/615), done.
Note: switching to '50712f4f2c1c68873ceba1c014d2032608b90fbb'.
...
Done, use the next commands to build the LArch product:  <4>
> cd /c/Users/user/git/larch
> mvn clean verify -s settings.xml

After a BUILD SUCCESS, the built products can be found at:
/c/Users/user/git/larch/products/nl.cge.mde.larch.package.product/target/products

Finally review the last commit and if all is ok, push it to ACIDSpace.
----
...
<2> Clone the MDE skeleton repository to download the bootstrap script.
    ...
    For a list of available versions, please see the https://gitlab.acidspace.nl/ec-mde/mde-skeleton/-/tags?sort=updated_desc&search=BOOTSTRAP[available tags].
...
<4> When the script is done, a commit has been created such that it can be pushed.
    The output provides information on how to build your product from the command line in order to verify it before pushing.
    Please read section <<setup-maven>> before running a command line build.
    When the commit is pushed, both the README of the repository and its Gitlab pages will contain information on how to setup an Eclipse development environment.
